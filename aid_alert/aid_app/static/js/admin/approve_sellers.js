    ...
    document.querySelectorAll('.btn-action, .btn-approve, .btn-reject').forEach(btn => {
        ...
        if (btn.classList.contains('btn-action')) {
            btn.addEventListener('click', function () {
                ...
                const sellerName = cells[1].textContent; // Business Name
                const sellerOwner = cells[2].textContent; // Owner

                const sellerStatus = row.querySelector('.badge') ? row.querySelector('.badge').textContent : 'Active';

                openSellerModal(sellerId, sellerName, sellerOwner, sellerStatus);
            });
        }
...
function openSellerModal(sellerId, sellerName, sellerOwner, sellerStatus) {
